...
with tf.name_scope("active_sensing_agent"):
    ...
    for t in range(tau):
        ...
        # Compute noiseless received signal for K samples
        # v1: (batch, N_rx, 1), H_eff: (batch, N_rx, N_tx), w1: (batch, N_tx, 1)
        g_eff = tf.matmul(tf.transpose(tf.conj(v1), perm=[0, 2, 1]), tf.matmul(H_eff, w1))  # (batch, 1, 1)
        g_eff = tf.reshape(g_eff, [-1, 1])  # (batch, 1)
        
        # Generate K samples with noise
        y_noiseless = tf.complex(tf.sqrt(lay['P']), 0.0) * g_eff  # (batch, 1)
        y_samples = []
        for k in range(K):
            noise_k = tf.complex(
                tf.random_normal([batch_size, 1], mean=0.0, stddev=noiseSTD_per_dim),
                tf.random_normal([batch_size, 1], mean=0.0, stddev=noiseSTD_per_dim)
            )
            y_k = y_noiseless + noise_k
            y_samples.append(y_k)
        
        # Stack and compute sufficient statistics (mean and power)
        y_stack = tf.stack(y_samples, axis=2)  # (batch, 1, K)
        y_mean = tf.reduce_mean(y_stack, axis=2)  # (batch, 1)
        y_power = tf.reduce_mean(tf.abs(y_stack) ** 2, axis=2)  # (batch, 1)
        ...
        for k in range(K):
            ...
            y_k_rev = y_noiseless_rev + noise_k_rev
            y_samples_rev.append(y_k_rev)
        
        y_stack_rev = tf.stack(y_samples_rev, axis=2)
        y_mean_rev = tf.reduce_mean(y_stack_rev, axis=2)
        y_power_rev = tf.reduce_mean(tf.abs(y_stack_rev) ** 2, axis=2)
