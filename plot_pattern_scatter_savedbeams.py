...
def compute_single_sinr_db(v_vec, w_vec, h_b, h_d, h_r, p_tx, noise_var=1.0):
    ...
    return 10.0 * np.log10(max(sinr, 1e-12))
...
n_show = np.random.choice(num_samples, size=1, replace=False)
...
for sample_idx in n_show:
    ...
    for step_idx in range(num_steps):
        ...
        ax.plot(angles, beam_r, color="#1f77b4", linewidth=2.0, label="Saved beam")
        ...
        ax.set_title(f"t={step_idx + 1}, SINR={sinr_steps_db[step_idx]:.2f} dB", fontsize=10)
        if step_idx == 0:
    ...
    v_opt = np.squeeze(np.asarray(data["v_optimal"][sample_idx]))
    w_opt = np.squeeze(np.asarray(data["w_optimal"][sample_idx]))
    optimal_final_sinr_db = compute_single_sinr_db(v_opt, w_opt, h_b, h_d, h_r, p_tx)
    ...
    beam_opt = joint_beam_pattern_db(
        v_opt,
        w_opt,
        angles,
    )
    ...
    if np.isfinite(true_bd_angle_deg) and -90.0 <= true_bd_angle_deg <= 90.0:
        theta_bd = np.deg2rad(true_bd_angle_deg)
        ax1.plot([theta_bd, theta_bd], [0, r_max], color="#2ca02c", linestyle="--", linewidth=1.8, label="BD")
